let problemData = [
    ...
    },
    {
        ...
        "problems": [
            ...
            {
                "id": 5,
                "input": "100\",
                "output": "200"
            }
        ]
    },
...
// }
var defaultUrl = "http://222.252.29.85:16358/";
var extraApiUrl = "http://222.252.29.85:16358/";
...
let problemInputFromData = problemData[0].problems.map(problem => problem.input).join("\n");
let problemOutputFromData = problemData[0].problems.map(problem => problem.output).join("\n");
...
function handleSubmit() {
    let actualOutput = stdoutEditor.getValue();
    let expectedOutput = problemOutputFromData.trim();
    let actualOutputArray = actualOutput.trim().split("\n");
    let expectedOutputArray = expectedOutput.split("\n");
    
    actualOutputArray.forEach((output, index) => {
        if (output === expectedOutputArray[index]) {
            $.ajax({
                url: 'http://192.168.148.70:1010/api/count-success',
                type: 'POST',
                success: function (response) {
                    console.log('API count_success được gọi thành công');
                },
                error: function (xhr, status, error) {
                    console.error('Lỗi khi gọi API count_success:', error);
                }
            });
        } else {
            $.ajax({
                url: 'http://192.168.148.70:1010/api//count-fail',
                type: 'POST',
                success: function (response) {
                    console.log('API count_fail được gọi thành công');
                },
                error: function (xhr, status, error) {
                    console.error('Lỗi khi gọi API count_fail:', error);
                }
            });
        }
    });
...
function runWithTestCases(input) {
